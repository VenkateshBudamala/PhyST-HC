...
def create_save_dir_and_save_scripts(
    ...
    for f in ["Run_Simulation.py", "Inputs.py", "Functions_Q.py", "Main_Module.py"]:
        src = code_dir / f
        if src.exists():
...
def start_console_logging(save_dir):
    log_file = Path(save_dir) / "Q_console.log"
...
def select_relevant_features(df_list: list[pd.DataFrame], target_col: str = 'Obs_Q',
    ...
    if drop_candidates is None:
        drop_candidates = ['Date', 'Obs_Q', 'year', 'day_of_year','FLOW_OUTcms']
    ...
    importance_df.to_excel(os.path.join(save_dir, "Q_Feature_Importance_RF.xlsx"), index=False)
        ...
        for df in df_list:
            ...
            if selected_features is not None:
                ...
                feature_cols = [
                    ...
                    if c not in ['Date', 'Obs_Q', 'day_of_year', 'month', 'year', 'GCN_Q']
                ]
            ...
            y = df["Obs_Q"].values.astype(float)
            mask = ~np.isnan(y)
...
def plot_hydrograph(results_df: pd.DataFrame, save_dir: str, station_id: str = None):
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(save_dir, f"Q_hydrograph_{station_id}.png"),
        dpi=300
    )
    plt.close()
...
def load_Parameter_Set(swat_path: str, excel_path: str, obs_to_reach_mapping: dict,
                       ...
                       warmup_years: int = 2) -> list[pd.DataFrame]:
    ...
    selected_cols_Q = [
        ...
        # Surface & subsurface flow components
        "SURQmm",     # surface runoff
        "LATQmm",     # lateral flow
        "GW_Qmm",     # groundwater flow
        "QTILEmm",    # tile drainage (important if present)
    
        # Soil–water state & percolation
        "SWmm",       # soil water storage
        "PERCmm",     # percolation
    
        # Water balance check (optional but useful)
        "WYLDmm"
    ]


    output_sub_df = output_sub_df[selected_cols_Q].copy()
    ...
    df_obs = pd.read_excel(excel_path, sheet_name='Q')
    ...
    for reach_id in reach_ids:
        ...
        if reach_id in df_obs.columns:
            swat_df = pd.merge(swat_df, df_obs[['Date', reach_id]].rename(columns={reach_id: "Obs_Q"}), on="Date", how="left")
        else:
            swat_df["Obs_Q"] = np.nan
        ...
        # FLOW routing memory
        swat_df["Q_lag1"] = swat_df["FLOW_OUTcms"].shift(1)
        swat_df["Q_lag2"] = swat_df["FLOW_OUTcms"].shift(2)
        
        # Drop rows consistently (VERY IMPORTANT)
        swat_df = swat_df.dropna(subset=["PRECIPmm_lag3","Q_lag2"]).reset_index(drop=True)
        
        swat_df.drop(columns=["month","Type"], inplace=True)
...
def prepare_dataset_pipeline(
    ...
    with open(os.path.join(save_dir, "Q_converted_reaches.txt"), "w") as f:
    ...
    try:
        ...
        importance_df.to_excel(
            os.path.join(save_dir, "Q_Feature_Importance_RF_fallback.xlsx"),
            index=False
        )
    ...
    with open(os.path.join(save_dir, "Q_selected_features.txt"), "w") as f:
...
def train_model_for_seq_len(
    ...
    for epoch in pbar:
        ...
        if current_val_loss < best_loss - min_delta:
            ...
            torch.save(model.state_dict(), os.path.join(save_dir, f"Q_best_model_seq{seq_len}.pt"))
            patience_counter = 0
        else:
            patience_counter += 1
...
# ).to(device)
    ...
    best_model.load_state_dict(torch.load(os.path.join(save_dir, f"Q_best_model_seq{seq_len}.pt")))
    best_model.eval()
    ...
    if plot_loss:
        ...
        plt.legend()
        plt.tight_layout()
        plt.savefig(os.path.join(save_dir, f"Q_training_history_seq{seq_len}.png"), dpi=300)
        plt.close()
    ...
    # --- Save to Excel ---
    train_pred_file = os.path.join(save_dir, f"Q_Calibration_Preds_seq{seq_len}.xlsx")
    ...
    for subbasin in df_preds_train['Subbasin'].unique():
        ...
        mask_valid = df_sub['Obs_Q'].notna() & df_sub['GCN_Q'].notna()
        ...
        if len(df_valid) == 0:
            ...
            obs_vals  = df_valid['Obs_Q'].values
            pred_vals = df_valid['GCN_Q'].values
            ...
            try:
                results_df = pd.DataFrame({
                    "Date": df_valid['Date'],
                    "Obs(m3/s)": df_valid['Obs_Q'],
                    "Pred(m3/s)": df_valid['GCN_Q'],
                ...
                plot_scatter(
                    ...
                    os.path.join(
                        sub_dir,
                        f"Q_scatter_seq{seq_len}_{phase_name.lower()}.png"
                    )
                )
    ...
    metrics_file = os.path.join(save_dir, f"Q_{phase_name}_Metrics_seq{seq_len}.xlsx")
...
def predict_and_evaluate_phase(
    ...
    output_file = os.path.join(save_dir, f"Q_{phase_name}_Preds_seq{seq_len}.xlsx")
        ...
        if len(df_valid) == 0:
            ...
            try:
                ...
                plot_scatter(obs_vals, pred_vals, f"Subbasin {subbasin} - Scatter Plot ({phase_name})",
                             os.path.join(sub_dir, f"Q_scatter_seq{seq_len}_{phase_name.lower()}.png"))
            except Exception as e:

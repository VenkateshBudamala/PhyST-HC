...
def create_save_dir_and_save_scripts(
    ...
    for f in ["Run_Simulation.py", "Inputs.py", "Functions_POC.py", "Main_Module.py"]:
        src = code_dir / f
        if src.exists():
...
def start_console_logging(save_dir):
    log_file = Path(save_dir) / "POC_console.log"
...
def select_relevant_features(df_list: list[pd.DataFrame], target_col: str = 'Obs_POC',
    ...
    if drop_candidates is None:
        drop_candidates = ['Date', 'Obs_POC', 'year', 'day_of_year','FLOW_OUTcms','GCN_POC']
    ...
    importance_df.to_excel(os.path.join(save_dir, "POC_Feature_Importance_RF.xlsx"), index=False)
        ...
        for df in df_list:
            ...
            if selected_features is not None:
                ...
                feature_cols = [
                    ...
                    if c not in ['Date', 'Obs_POC', 'day_of_year', 'month', 'year', 'GCN_POC']
                ]
            ...
            y = df["Obs_POC"].values.astype(float)
            mask = ~np.isnan(y)
...
def plot_hydrograph(results_df: pd.DataFrame, save_dir: str, station_id: str = None):
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(save_dir, f"POC_hydrograph_{station_id}.png"),
        dpi=300
    )
    plt.close()
...
def load_Parameter_Set(swat_path: str, excel_path: str, obs_to_reach_mapping: dict,
    ...
    selected_cols = [
        ...
        # =========================
        "PRECIPmm",      # Rainfall forcing (both)
        "SURQmm",        # Surface runoff (critical for POC)
        "LATQmm",        # Lateral flow (critical for POC)
        "GW_Qmm",        # Groundwater contribution (POC)
        "SYLDt/ha",      # Sediment yield (POC driver)
        "SedCkg/ha",     # Sediment-bound carbon (POC)
        "SWmm",          # Soil moisture state (both)
        "AirTempC",      # Temperature control (POC production)
    
        # =========================
        # 🟡 MEDIUM IMPACT (Supporting Controls)
        # =========================
        "PERCmm",        # Percolation (POC pathway)
        "WYLDmm",        # Total water yield (summary transport)
        "ORGNkg/ha",     # Organic nutrient export (POC-related)
        "ETmm",          # Evapotranspiration (moisture modifier)
        "NPPCkg/ha",     # Carbon production
        "RspCkg/ha",     # Soil respiration
    
        # =========================
        # 🔵 LOWER IMPACT (Secondary / Optional)
        # =========================
        "AREAkm2",       # Static catchment descriptor
        "Subbasin"       # Spatial index
    ]
    ...
    df_obs = pd.read_excel(excel_path, sheet_name='POC')
    ...
    for reach_id in reach_ids:
        ...
        if reach_id in df_obs.columns:
            swat_df = pd.merge(swat_df, df_obs[['Date', reach_id]].rename(columns={reach_id: "Obs_POC"}), on="Date", how="left")
        else:
            swat_df["Obs_POC"] = np.nan
...
def prepare_dataset_pipeline(
    ...
    with open(os.path.join(save_dir, "POC_converted_reaches.txt"), "w") as f:
    ...
    try:
        selected_features, importance_df = select_relevant_features(
            ...
            drop_candidates=['Date', 'Obs_POC', 'year', 'day_of_year','FLOW_OUTcms','GCN_Q'],
            features_to_add = features_to_add
        )
        ...
        importance_df.to_excel(
            os.path.join(save_dir, "POC_Feature_Importance_RF_fallback.xlsx"),
            index=False
        )
    ...
    with open(os.path.join(save_dir, "POC_selected_features.txt"), "w") as f:
...
def train_model_for_seq_len(
    ...
    for epoch in pbar:
        ...
        if current_val_loss < best_loss - min_delta:
            ...
            torch.save(model.state_dict(), os.path.join(save_dir, f"POC_best_model_seq{seq_len}.pt"))
            patience_counter = 0
        else:
            patience_counter += 1
    ...
    best_model.load_state_dict(torch.load(os.path.join(save_dir, f"POC_best_model_seq{seq_len}.pt")))
    best_model.eval()
    ...
    if plot_loss:
        ...
        plt.legend()
        plt.tight_layout()
        plt.savefig(os.path.join(save_dir, f"POC_training_history_seq{seq_len}.png"), dpi=300)
        plt.close()
    ...
    # --- Save to Excel ---
    train_pred_file = os.path.join(save_dir, f"POC_Calibration_Preds_seq{seq_len}.xlsx")
    ...
    for subbasin in df_preds_train['Subbasin'].unique():
        ...
        mask_valid = df_sub['Obs_POC'].notna() & df_sub['GCN_POC'].notna()
        ...
        if len(df_valid) == 0:
            ...
            obs_vals  = df_valid['Obs_POC'].values
            pred_vals = df_valid['GCN_POC'].values
            ...
            try:
                results_df = pd.DataFrame({
                    "Date": df_valid['Date'],
                    "Obs(kg/day)": df_valid['Obs_POC'],
                    "Pred(kg/day)": df_valid['GCN_POC'],
                ...
                plot_scatter(
                    ...
                    os.path.join(
                        sub_dir,
                        f"POC_scatter_seq{seq_len}_{phase_name.lower()}.png"
                    )
                )
    ...
    metrics_file = os.path.join(save_dir, f"POC_{phase_name}_Metrics_seq{seq_len}.xlsx")
...
def predict_and_evaluate_phase(
    ...
    output_file = os.path.join(save_dir, f"POC_{phase_name}_Preds_seq{seq_len}.xlsx")
        ...
        if len(df_valid) == 0:
            ...
            try:
                ...
                plot_scatter(obs_vals, pred_vals, f"Subbasin {subbasin} - Scatter Plot ({phase_name})",
                             os.path.join(sub_dir, f"POC_scatter_seq{seq_len}_{phase_name.lower()}.png"))
            except Exception as e:
